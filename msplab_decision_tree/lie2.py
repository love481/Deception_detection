...
# 1. action_unit_emotion_lie_predict_from_n_frames(list_images):
#     input: list of n raw RGB images as list of matrix 480x640x3 where n is multiple of 30(fps)
#     outputs: None or AUs  (41 Ekman Action Units both unilateral and bilateral as multi class labeling with each either 0 or 1)
...
#                   indexed by (n/30) sec in the video
#                    so that:
#                         1st item in the list corresponds to (n/30) sec in video 
#                         2nd item in the list corresponds to (n/30)*2 sec in video etc
...
class show():
    ...
    def pred(self):
        ...
        max_sum = window_sum
        # Action unit labels
        max_sum=(window_sum>(self.window_size*0.90)).astype(np.int32)
        return max_sum.tolist()
    def run(self):
        AU_list  = self.pred()
        return AU_list
        ...
        for i,img in enumerate(list_images):
            if i<args.len_cut:
                ...
                if(len(output_points)):
                    ...
                    logps=lnd_AU.run()
                    logps=np.array(logps>0.5).astype(np.int32)
                    self.AU_store(logps) 
        if len(self.log) == args.len_cut:
            self.show_obj.set_inputs(self.log)
            logps=self.show_obj.run() 
            return logps
        else:
            return None
...
def action_unit_emotion_lie_predict_from_n_frames(list_images):
    api=lie()
